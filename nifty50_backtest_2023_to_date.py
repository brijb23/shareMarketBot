...
# -*- coding: utf-8 -*-
"""
NIFTY50 BACKTEST 2023 TO DATE - WEEKLY
Backtests trading recommendations from 2023 till present
...
class BacktestLogger:
    def __init__(self, name):
        ...
        self.log_file = Config.LOG_DIR / f'BACKTEST_{self.timestamp}.txt'
        self.messages = []
...
logger = BacktestLogger('BACKTEST')
...
class PortfolioManager:
    def __init__(self, initial_capital):
        ...
        self.positions = {}  # ticker -> {entry_price, qty, entry_date, cost, target, stop_loss, last_update}
        self.trades = []
    ...
    def execute_buy(self, ticker, entry_price, target, stop_loss, signal_date, allocation):
        ...
        self.positions[ticker] = {
            ...
            'target': target,
            'stop_loss': stop_loss,
            'last_update': signal_date
        }
    
    def update_position(self, ticker, new_target, new_stop_loss, update_date):
        """Update target and stop loss for existing position"""
        if ticker in self.positions:
            self.positions[ticker]['target'] = new_target
            self.positions[ticker]['stop_loss'] = new_stop_loss
            self.positions[ticker]['last_update'] = update_date
    
    def check_positions_for_exits(self, recommendations, current_date):
        """Check if any positions hit stop loss or target
        Returns separate lists for SL hits and target hits"""
        sl_hits = []
        target_hits = []
        ...
        for ticker, position in self.positions.items():
            ...
            if current_price <= position['stop_loss']:
                sl_hits.append((ticker, current_price))
            # Check if hit target (soft stop, check analysis first)
            elif current_price >= position['target']:
                target_hits.append((ticker, current_price))
        
        return sl_hits, target_hits
    
    def close_positions_by_sl(self, sl_hits, current_date):
        """Close positions that hit stop loss"""
        for ticker, exit_price in sl_hits:
            self.execute_sell(ticker, exit_price, current_date, 'STOP_LOSS')
        return len(sl_hits)
    
    def close_positions_by_target(self, target_hits, recommendations, current_date):
        """Close positions that hit target only if target hasn't improved
        If target improved, update position instead"""
        closed_count = 0
        for ticker, exit_price in target_hits:
            ...
            if ticker in recommendations:
                ...
                new_target = rec['target']
                old_target = position['target']
                
                # If new target is higher, update and hold
                if new_target > old_target:
                    new_stop_loss = rec['stop_loss']
                    self.update_position(ticker, new_target, new_stop_loss, current_date)
                    # Don't close, just update
                else:
                    # Target hasn't improved, sell at current price
                    self.execute_sell(ticker, exit_price, current_date, 'TARGET')
                    closed_count += 1
...
class BacktestRunner:
    ...
    def run(self):
        ...
        try:
            self.logger.log("\n" + "="*70)
            self.logger.log("NIFTY50 BACKTEST 2023 TO DATE - STARTING")
            self.logger.log("="*70)
            ...
            for i, analysis_date in enumerate(week_ends, 1):
                try:
                    ...
                    # Get recommendations for this date (FIRST - analyze before closing)
                    ...
                    # Step 3a: Check which positions hit stop loss or target
                    sl_hits, target_hits = self.portfolio.check_positions_for_exits(recommendations, analysis_date)
                    
                    # Step 3b: Close positions that hit stop loss (hard stop, no discussion)
                    if sl_hits:
                        closed_sl = self.portfolio.close_positions_by_sl(sl_hits, analysis_date)
                        self.logger.log(f"  Closed {closed_sl} positions (Stop Loss hit)")
                    
                    # Step 3c: Close positions that hit target (soft stop - check analysis first)
                    # If target improved, update and hold instead of selling
                    if target_hits:
                        closed_target = self.portfolio.close_positions_by_target(target_hits, recommendations, analysis_date)
                        updated_count = len(target_hits) - closed_target
                        if closed_target > 0:
                            self.logger.log(f"  Closed {closed_target} positions (Target hit, no improvement)")
                        if updated_count > 0:
                            self.logger.log(f"  Updated {updated_count} positions (Target improved, holding)")
                    
                    # Step 3d: Update existing positions with new targets/stop losses (for positions not at target)
                    for ticker, position in list(self.portfolio.positions.items()):
                        if ticker in recommendations:
                            rec = recommendations[ticker]
                            # Update to new target and stop loss from latest analysis
                            new_stop_loss = rec['stop_loss']
                            new_target = rec['target']
                            
                            # Only update if new target is better
                            if new_target > position['target']:
                                self.portfolio.update_position(ticker, new_target, new_stop_loss, analysis_date)
                                self.logger.log(f"  Updated {ticker}: Target {position['target']:.2f} -> {new_target:.2f}")
                    
                    # Step 3e: Execute NEW BUY signals only for stocks not already held
    ...
    def generate_outputs(self):
        ...
        # CSV output
        csv_file = Config.OUTPUT_DIR / f'BACKTEST_WEEKLY_{timestamp}.csv'
        if self.portfolio.monthly_results:
        ...
        # Trades CSV
        trades_file = Config.OUTPUT_DIR / f'BACKTEST_TRADES_WEEKLY_{timestamp}.csv'
        ...
        # JSON output
        json_file = Config.OUTPUT_DIR / f'BACKTEST_RESULTS_WEEKLY_{timestamp}.json'
        output = {
            'metadata': {
                'generated': timestamp,
        ...
        # Summary text
        summary_file = Config.OUTPUT_DIR / f'BACKTEST_SUMMARY_WEEKLY_{timestamp}.txt'
        with open(summary_file, 'w') as f:
            f.write("NIFTY50 BACKTEST SUMMARY (WEEKLY)\n")
            f.write("="*60 + "\n\n")
            f.write(f"Initial Capital: {Config.INITIAL_CAPITAL:,} INR\n")
            f.write(f"Period: 2023-01-01 to {datetime.now().strftime('%Y-%m-%d')}\n")
            ...
            if self.portfolio.monthly_results:
                ...
                f.write("-"*60 + "\n")
                for result in self.portfolio.monthly_results:
